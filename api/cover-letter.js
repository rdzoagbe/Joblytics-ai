...
async function handleCvOptimize(req, res) {
  ...
  const missingKeywords = (r.keyword_match?.missing_required || r.keyword_match?.missing || []).slice(0, 12)
  const foundKeywords = (r.keyword_match?.found || r.keyword_match?.matched || []).slice(0, 12)
  const criticalGaps = (r.gaps_to_address || r.critical_gaps || []).slice(0, 5)
  const prompt = `You are a senior career coach producing a job seeker's COMPLETE, ready-to-send resume for a specific job. This is a full replacement resume they will download and send — their real identity and full work history with your improvements applied — NOT a summary of changes and NOT a list of missing keywords.\n\nYou must NOT fabricate or invent experience that isn't in the original CV. You may only: reword existing experience using more impactful language, reorganize to put the most job-relevant content first, emphasize job keywords that are already present, and weave in the missing keywords ONLY where the candidate's real experience already supports them.\n\nReturn a STRUCTURED JSON object with these exact fields. ${langInstruction}\n\nJOB CONTEXT:\nTitle: ${r.job_context?.title || 'Position'}\nCompany: ${r.job_context?.company || 'Not specified'}\nRequired keywords (already in CV): ${foundKeywords.join(', ') || 'none extracted'}\nMissing keywords (integrate ONLY where truthful): ${missingKeywords.join(', ') || 'none'}\nCritical gaps to address: ${criticalGaps.join('; ') || 'none'}\n\nORIGINAL CV (this is the candidate's real resume — preserve their identity and every role):\n${cvText.slice(0, 14000)}\n\nOUTPUT FORMAT — return ONLY this JSON, no preamble, no markdown. Fill EVERY field from the real CV; never leave name or contact blank:\n{\n  "header": { "full_name": "<the candidate's real full name, exactly as in the CV>", "title": "<their professional title>", "contact": { "email": "<their email from the CV>", "phone": "<their phone from the CV>", "location": "<their city/region from the CV>", "linkedin": "<their LinkedIn URL from the CV, or empty>" } },\n  "summary": "<3-4 sentence professional summary tailored to the target role, grounded in their real background>",\n  "experience": [{ "title": "<real role title>", "company": "<real company>", "location": "<location>", "dates": "<real dates>", "bullets": ["<improved bullet grounded in the CV>", "3 to 5 bullets per role — keep the real detail, don't drop content"] }],\n  "skills": { "technical": [], "soft": [], "languages": [] },\n  "education": [{ "degree": "<real degree>", "institution": "<real institution>", "location": "", "dates": "" }],\n  "changes_made": ["<change 1>", "<change 2>", "<change 3>"],\n  "honest_disclaimer": "<one short sentence reminding user to verify before sending>"\n}\n\nCRITICAL RULES: NEVER invent skills, jobs, or qualifications. Extract the candidate's REAL full_name and contact details (email, phone, location, LinkedIn) from the CV — never leave them blank or use a placeholder. Include EVERY real role from the CV (up to 8, most recent first) with 3-5 bullets each, and ALL education — the resume must be complete, not a fragment. Maximum 10 technical skills, 6 soft skills.`
  let message
  try {
    message = await client.messages.create({ model: 'claude-sonnet-4-6', max_tokens: 4096, temperature: 0.3, messages: [{ role: 'user', content: prompt }] })
  } catch (e) {
  ...
  optimized.header.contact = optimized.header.contact || {}
  // Safety net: never ship a resume with a blank name — fall back to the CV's first line.
  if (!String(optimized.header.full_name || '').trim()) {
    const firstLine = cvText.split('\n').map(s => s.trim()).find(Boolean) || ''
    optimized.header.full_name = firstLine.slice(0, 80)
  }
  optimized.experience = Array.isArray(optimized.experience) ? optimized.experience.slice(0, 8) : []
